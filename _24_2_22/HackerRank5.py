...
for i in range(m):
    str1 = input()
    op=[str(val1) for val1 in str1.split() if val1.isdigit()==False]
    val=[int(val1) for val1 in str1.split() if val1.isdigit()==True]
    val1=set(map(int,(input().split())))
    if op[0]=='intersection_update':
        A.intersection_update(val1)
        print("A=",A)
    if op[0] =='update':
        A.update(val1)
        print("A=",A)
    if op[0] =='symmetric_difference_update':
        A.symmetric_difference_update(val1)
        print("A=",A)
    if op[0] =='difference_update':
        A.difference_update(val1)
        print("A=",A)
print(sum(A))
